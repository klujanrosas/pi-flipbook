...
import { detectVideoPaths } from "../src/detect-paths.ts";
...
test("finds multiple videos in one message, in document order", () => {
	...
	for (let i = 1; i < results.length; i++) {
		expect(results[i]!.start, `match ${i} overlaps match ${i - 1}`).toBeGreaterThanOrEqual(results[i - 1]!.end);
	}
});

test("does NOT match non-video extensions (images, audio, etc.)", () => {
	const samples = ["/tmp/shot.png what is this", "look at /tmp/song.mp3", "/tmp/image.gif", "/tmp/notes.txt"];
